...
while True:
    ...
    for rect in rects: # 얼굴 탐지
        ...
        if both_ear < EAR_THRESH :  # EAR이 임계치 이하이면 한 frame당 counter 증가
            if not TIMER_FLAG:
                start_closing = timeit.default_timer()  #눈 감기기 시작한 시간
                TIMER_FLAG = True
            COUNTER += 1
            CHECK_FRAME += 1
            
            if COUNTER >= EAR_CONSEC_FRAMES:  # frames = 90, counter가 90이상이면 3초이상 눈 감는다.
                if COUNTER == 90:  # 3초 이상 눈감은 상태
                    COUNTER_ONE += 1
                if CHECK_FRAME <= 1800:  # 1분당 3회 이상 발생시 졸음
                    if COUNTER_ONE >= 3:
                        mid_closing = timeit.default_timer()  # 90frame 이후까지 눈 감은 시간
                        closing_time = round((mid_closing-start_closing),3)             
                else:
                    CHECK_FRAME = 0
                    COUNTER_ONE = 0
                    if closing_time >= RUNNING_TIME:  # 알람이 울리기 시작하는 시간
                        if RUNNING_TIME == 0 :
                            CUR_TERM = timeit.default_timer()
                            OPENED_EYES_TIME = round((CUR_TERM - PREV_TERM),3)
                            PREV_TERM = CUR_TERM
                            RUNNING_TIME = 1.75  # 졸음단계 1,2 사이
